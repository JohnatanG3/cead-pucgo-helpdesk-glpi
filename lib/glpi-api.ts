...
export async function getTicketFollowups(
	...
	try {
		...
		const response = await fetch(
			`${GLPI_API_URL}/Ticket/${ticketId}/TicketFollowup`,
			{
				headers: {
					"Content-Type": "application/json",
					"App-Token": GLPI_APP_TOKEN,
		...
		if (!response.ok) {
			console.error(
				`Erro ao obter followups do ticket ${ticketId}: ${response.statusText}`,
			);
...
export function mapGLPIPriorityToString(
	priority: number,
 ...
 */
export async function getTicketsByGroup(
	groupId: number,
	params: Record<string, string> = {},
): Promise<GLPITicket[]> {
...
export async function getTicketStats(): Promise<{
	...
	allTickets.forEach((ticket) => {
		...
		if (ticket.groups_id_assign) {
			...
			stats.byAssignedGroup[groupId] =
				(stats.byAssignedGroup[groupId] || 0) + 1;
		}
...
// Adicione estas funções ao arquivo lib/glpi-api.ts

// Função para obter documentos de um ticket
export async function getTicketDocuments(
	...
	try {
			...
			return [
				{
					...
					filepath: "/uploads/documento1.pdf",
		...
		const sessionToken = await initSession();
		const response = await fetch(
			`${GLPI_API_URL}/Ticket/${ticketId}/Document_Item`,
			{
				headers: {
					"Content-Type": "application/json",
					"App-Token": GLPI_APP_TOKEN,
		...
		if (!response.ok) {
			throw new Error(
				`Erro ao obter documentos do ticket ${ticketId}: ${response.statusText}`,
			);
		}

		const documentItems = await response.json();

		// Para cada Document_Item, buscar os detalhes do documento
		const documents = await Promise.all(
			// biome-ignore lint/suspicious/noExplicitAny: <explanation>
			documentItems.map(async (item: any) => {
				const docResponse = await fetch(
					`${GLPI_API_URL}/Document/${item.documents_id}`,
					{
						headers: {
							"Content-Type": "application/json",
							"App-Token": GLPI_APP_TOKEN,
				...
				if (!docResponse.ok) {
					throw new Error(
						`Erro ao obter documento ${item.documents_id}: ${docResponse.statusText}`,
					);
				}
		...
		console.error(`Erro ao obter documentos do ticket ${ticketId}:`, error);
		return []; // Retornar array vazio em caso de erro
	}
}
...
export async function deleteDocument(documentId: number): Promise<void> {
	try {
		// Em ambiente de desenvolvimento, simular exclusão
		if (process.env.NODE_ENV === "development") {
			console.log(`Simulando exclusão do documento ${documentId}`);
			return;
		}

		// Em produção, excluir da API
		const sessionToken = await initSession();
		const response = await fetch(`${GLPI_API_URL}/Document/${documentId}`, {
			method: "DELETE",
			headers: {
				"Content-Type": "application/json",
				"App-Token": GLPI_APP_TOKEN,
		...
		if (!response.ok) {
			throw new Error(
				`Erro ao excluir documento ${documentId}: ${response.statusText}`,
			);
		}
	} catch (error) {
