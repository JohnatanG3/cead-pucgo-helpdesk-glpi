...
import { createTicket, uploadDocument, linkDocumentToTicket, getCategories, handleApiRequest } from "@/lib/glpi-api"
...
export default function NewTicketPage() {
  ...
  useEffect(() => {
    handleApiRequest(() => getCategories(), "Não foi possível carregar as categorias")
      .then((data) => {
        setCategories(data)
      })
      .finally(() => {
      ...
      if (files.length > 0) {
        try {
          for (const file of files) {
            const document = await uploadDocument(file, user?.id ? Number(user.id) : 1)
  ...
  const renderPriorityValue = () => {
    if (!priority) return <span className="text-muted-foreground">Selecione a prioridade</span>
      ...
      <main className="flex-1 p-4 md:p-6">
        <div className="container mx-auto max-w-3xl">
          <div className="mb-6">
            ...
            <p className="text-muted-foreground">Preencha as informações abaixo para criar um novo chamado.</p>
          </div>
          ...
          <Card>
            ...
            <form onSubmit={handleSubmit}>
              <CardContent className="space-y-4">
                {error && <ErrorMessage message="Erro ao criar chamado" details={error} severity="error" />}
              ...
              <CardFooter className="flex justify-between gap-4 pt-6 border-t">
                ...
                >
                  {isLoading ? <LoadingSpinner size="sm" text="Enviando chamado..." /> : "Criar Chamado"}
                </Button>
              </CardFooter>
            </form>
          </Card>
