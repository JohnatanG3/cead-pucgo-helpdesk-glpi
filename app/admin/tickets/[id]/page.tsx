...
export default function AdminTicketDetailPage() {
	...
	useEffect(() => {
		console.log("AdminTicketDetailPage - Verificando autenticação", {
			user,
			authLoading,
		});
		...
		if (!authLoading) {
			if (!user) {
				console.log(
					"AdminTicketDetailPage - Usuário não autenticado, redirecionando para login",
				);
				router.push("/");
			} else if (user.role !== "admin") {
				console.log(
					"AdminTicketDetailPage - Usuário não é admin, redirecionando para dashboard",
				);
				toast.error("Você não tem permissão para acessar esta página.");
				router.push("/dashboard");
			} else {
				console.log(
					"AdminTicketDetailPage - Usuário autenticado como admin, carregando ticket",
				);
				loadTicket();
			}
		}
		...
		try {
			...
			const ticketData = await Promise.race([
				...
				new Promise((_, reject) =>
					setTimeout(
						() => reject(new Error("Timeout ao carregar ticket")),
						8000,
					),
				),
			]);
			...
			if (!ticketData) {
				console.error(
					`AdminTicketDetailPage - Ticket não encontrado: ${ticketId}`,
				);
				setError("Chamado não encontrado.");
				toast.error("Chamado não encontrado.");
			} else {
				console.log(
					"AdminTicketDetailPage - Ticket carregado com sucesso:",
					ticketData,
				);
			...
			<div className="flex min-h-screen items-center justify-center">
				<div className="text-center">
					<h2 className="text-2xl font-bold text-red-500 mb-4">
						{timeoutOccurred ? "Tempo limite excedido" : "Erro"}
					</h2>
