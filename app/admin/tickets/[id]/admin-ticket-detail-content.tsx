...
import { ArrowLeft, Clock, FileText, User, Edit2, Trash2, Paperclip, X, AlertTriangle } from "lucide-react"
...
export function AdminTicketDetailContent({ ticket }: { ticket: any }) {
  ...
  const [editedPriority, setEditedPriority] = useState<number>(ticket?.priority || 3)
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      ticket.priority = editedPriority
                            ...
                            <Avatar className="h-8 w-8">
                              <AvatarFallback>
                                {followup.users_id === ticket.users_id_recipient
                                  ? getEmailInitial(followup.user?.email || "Usuário")
                                  : getEmailInitial(followup.user?.email || "Atendente")}
                              </AvatarFallback>
                            </Avatar>
                            <div>
      ...
      <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
        <DialogContent className="sm:max-w-[600px] max-h-[90vh] flex flex-col">
            ...
            <div className="space-y-2 pt-4 border-t">
              ...
              {isLoadingDocuments ? (
                <div className="text-center py-2">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">Carregando anexos...</p>
                </div>
              ) : ticketDocuments.length > 0 ? (
                  ...
                  {ticketDocuments.map((doc) => (
                    <div key={doc.id} className="flex items-center justify-between p-2 border rounded-md">
                      ...
                      {documentsToDelete.includes(doc.id) ? (
                        <div className="flex items-center gap-2">
                          <span className="text-xs text-red-500">Será excluído</span>
                          <Button
                            type="button"
                            variant="ghost"
                            size="sm"
              ...
              <div className="mt-4">
                ...
                <p className="text-xs text-muted-foreground mt-1">Todos os tipos de arquivos são aceitos (máx. 5MB)</p>
              </div>
            </div>
          </div>
          <DialogFooter className="pt-2 border-t mt-2">
            ...
            <Button onClick={handleSaveEdit} disabled={isEditing || !editedTitle.trim() || !editedContent.trim()}>
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              <div className="flex flex-col gap-2">
                ...
                <p>
                  Tem certeza que deseja excluir permanentemente o chamado #{ticket.id} - {ticket.name}?
                </p>
                <p>Todos os dados relacionados a este chamado, incluindo respostas e anexos, serão excluídos.</p>
              </div>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
